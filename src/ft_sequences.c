...
/*   Updated: 2017/12/18 18:13:03 by pguillie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_sequence4(double *c, double *z, int iterations)
{
	double	t[2];
	int		i;
	...
	while (i < iterations && z[0] * z[0] + z[1] * z[1] < 4)
	{
		t[0] = (z[0] < 0 ? -z[0] : z[0]);
		t[1] = (z[1] < 0 ? -z[1] : z[1]);
		z[0] = t[0] * t[0] * t[0] - 3 * t[0] * t[1] * t[1] + c[0];
		z[1] = 3 * t[0] * t[0] * t[1] - t[1] * t[1] * t[1] + c[1];
		i++;
	}
	return (i);
}
